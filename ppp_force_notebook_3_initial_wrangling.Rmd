...
Just for fun, let's jump ahead and see how sum of force applications associates with perceived mental status:
undone undone
```{r}
CrossTable(ppb_force$force_officer_sum, ppb_force$civil_mental, chisq = TRUE, simulate.p.value = TRUE)
```
```{r}
mytable <- xtabs(~force_officer_sum + civil_mental, data=ppb_force
                 )
...
chisq.test(ppb_force$civil_mental, ppb_force$force_officer_sum, simulate.p.value = TRUE)
```
```{r}
mytable <- table(ppb_force$civil_mental, ppb_force$force_officer_sum)
ftable(mytable)   #this command said to allow more attractive printing
```
```{r}
hist(ppb_force$force_officer_sum)
```
```{r}
# Filled Density Plot
d <- density(ppb_force$force_officer_sum)
...
t.test(ppb_force$force_officer_sum ~ ppb_force$civil_mental)
```
sapply(ppb_force$, mean, na.rm=TRUE)

```{r}
class_summary <- ppb_force %>% 
#  filter(!is.na(ht_in)) %>% 
  group_by(ppb_force$civil_mental) %>% 
  summarise(
    n                    = n(),
